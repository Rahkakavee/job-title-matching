...
As pointed out in the literature review \ac{NB}, \ac{MLR} and \ac{SVM} have several advantages for text classifcation tasks. In the following based on a theoretical discussion of each classifier, the exact modeling of the classifiers is justified. 
...
\begin{figure}[hb!]
  ...
  \caption{\label{fig: F7} Multinomial Logistic Regression}
\end{figure}
...
Then the output vector y can be calculated by $\hat{y} = softmax(\textbf{W}x+b)$. 

Cosidering the training of the weights and the bias the goal is to commonly cross-entropy-loss is applied for calculating the error while optimizing. 
