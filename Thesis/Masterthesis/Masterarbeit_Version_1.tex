...
In contrast to the previous two classifiers \ac{RF} is a ensemble learning technique. The main idea of ensemble learning techniques is to create a number of learners, classifiers, and combine them. Usually those learners are for example descision tree or neural networks and are usuall homogeneous, which means that each individual learn is based on the same machine learning algorithm. The different ensemble technique are built on three pillars: the data sampling, the strategy of the training and combination method \cite{polikar2012, zhou2009}. 
...
\[ I_{gini}(X) = \sum_{i=1}^k p_i(1-p_i) \]

where X is a set of training examples, K is th set classes with labels from 1,...k and $p_i$ is the probability of examples belong to a certain class.

The algorithm stops for a specific node if all examples of the training set belong to the same class, or if there are no features left for splitting. This is can end in tree with a high depth, which is why often pruning is applied to avoid overfitting of the tree \citet{Berthold2020}.

Based on above theoretical explanations of the foundations of decision tree, researchers have developed several algorithms to train decision trees, like Iterative Dichotomiser 3, C4.5. or \ac{CART}, which is used in \ac{RF}. \ac{CART} produces depending on the target variable classification (for categorical variables) or regression trees (for numerical variables). It constructs only binary trees, thus each each split is into two nodes. The algorithm uses as impurity measurement gini index and it can handle numerical and categorical input \citep{brijain2014}.
